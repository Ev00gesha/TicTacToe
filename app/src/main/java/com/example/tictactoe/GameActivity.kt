...
class GameActivity : AppCompatActivity() {
    ...
    private lateinit var settingInfo: SettingsActivity.InfoSettings
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        settingInfo = getSettingsInfo()

        mediaPlayer = MediaPlayer.create(this, R.raw.song2)
        mediaPlayer.isLooping = true
        setVolumeMediaPlayer(settingInfo.sound)
        mediaPlayer.start()
    ...
    private fun makeStepOfUser(row: Int, column: Int){
        if(isEmptyField(row, column)){
            ...
            if(!isFilledGame()){
                val result = makeStepOfAI()

                val statusAi = checkGameField(result.row, result.column, "0")
                if(statusAi.status) {
            ...
            Toast.makeText(this, "Ячейка уже занята", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun makeStepOfAI() : CellGameField{
        return when(settingInfo.level){
            0 -> makeStepOfAIEasyLevel()
            1 -> makeStepOfAIMediumLevel()
    ...
    private fun makeStepOfAIHardLevel() : CellGameField{
    ...
    private fun makeStepOfAIMediumLevel() : CellGameField {
        var bestScore = Double.NEGATIVE_INFINITY
        var move = CellGameField(0, 0)
    ...
    private fun makeStepOfAIEasyLevel() : CellGameField{
        var randRow = 0
        var randColumn = 0
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if(resultCode == REQUEST_POPUP_MENU){
    ...
    private fun showPopupMenu(){
        ...
        val toSaveExit = dialog.findViewById<TextView>(R.id.dialog_save_exit)
    ...
    private fun getSettingsInfo(): SettingsActivity.InfoSettings {
        with(getSharedPreferences(getString(R.string.preference_file_key), MODE_PRIVATE)){
            val sound = getInt(SettingsActivity.PREF_SOUND, 30)
